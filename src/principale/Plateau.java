...
public class Plateau {
	...
	public int testJeton (Coord coord,Jeton j) {
		String sVertical = Character.toString(j.get_lettre());
		String sHorizontal = Character.toString(j.get_lettre());
		int bonusHorizontal = 0,bonusVertical = 0,scoreVertical = 0,scoreHorizontal = 0,scoreFinal=0;
		Coord copy = new Coord (coord.a,coord.b);
		while ((copy.a<15)&&(getCase(copy).getJeton()!=null))
		{
			int bonus;
			copy.a++;
			Case c = getCase(copy);
			switch (c.get_bonus()) {
			case lettrecomptedouble : bonus=2;break;
			case lettrecomptetriple : bonus=3;break;
			case motcomptedouble : bonusVertical+=2;
			case motcomptetriple : bonusVertical+=3;
			default : bonus = 1;break;
			}
			sVertical=sVertical+Character.toString(c.getJeton().get_lettre());
			scoreVertical+=(c.getJeton().get_score())*bonus;
		}
		copy = new Coord (coord.a,coord.b);
		while((copy.a>0)&&(getCase(copy).getJeton()!=null))
		{
			int bonus;
			copy.a--;
			Case c = getCase(copy);
			...
			sVertical=Character.toString(c.getJeton().get_lettre())+sVertical;
			scoreVertical+=(c.getJeton().get_score())*bonus;
		}
		if (bonusVertical==0) bonusVertical=1;
		scoreVertical*=bonusVertical;
		copy = new Coord (coord.a,coord.b);
		while ((copy.a<15)&&(getCase(copy).getJeton()!=null))
		{
			int bonus;
			...
			case lettrecomptedouble : bonus=2;break;
			case lettrecomptetriple : bonus=3;break;
			case motcomptedouble : bonusHorizontal+=2;
			case motcomptetriple : bonusHorizontal+=3;
			default: bonus = 1;break;
			}
			sHorizontal=sHorizontal+Character.toString(getCase(copy).getJeton().get_lettre());
			scoreHorizontal+=(c.getJeton().get_score())*bonus;
		}
		copy = new Coord (coord.a,coord.b);
		for (int i=0;(copy.a<15)&&(getCase(copy).getJeton()!=null);i++)
		{
			int bonus;
			copy.a++;
			Case c = getCase(copy);
			...
			sHorizontal=Character.toString(getCase(copy).getJeton().get_lettre())+sHorizontal;
			scoreHorizontal+=(c.getJeton().get_score())*bonus;
		}
		if (bonusHorizontal==0) bonusHorizontal=1;
		scoreHorizontal*=bonusHorizontal;
		if (d.searchMot(sVertical)&&d.searchMot(sHorizontal)) {
			scoreFinal = scoreHorizontal+scoreVertical;
			if (scoreFinal!=0)
				scoreFinal+=j.get_score();
			return scoreFinal;
		}
		return 0;
	}
	...
	public ArrayList<Jeton> piocherColl (int i) {
		return s.piocher(i);
	}
